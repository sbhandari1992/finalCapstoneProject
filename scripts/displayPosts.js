...
export function displayPosts(specificUser) {
  ...
  fetch(endpoint + `posts?limit=1000`, options)
    ...
    .then((data) => {
      if (specificUser)
        data = data.filter((arItm) => specificUser === arItm.username);
      data.forEach((arrItem) => {
        let postDate = arrItem.createdAt;
        let postDateConvertedToCurrentDate = new Date(postDate);
        let convertedPostTime = new Date(arrItem.createdAt);
        let options = {
          hour: "numeric",
          minute: "numeric",
          hour12: true,
        };
        const hourAndMinute = convertedPostTime.toLocaleString("en-US", options);
        ...
        timeSpan.innerText = `Posted on ${(postDateConvertedToCurrentDate.toDateString())} ${hourAndMinute}`;

        // timeSpan.innerText = `Posted on ${timeArray[0]}/${timeArray[1]}/${splitDay[0] }  ${splitHour[0]}:${splitHour[1]}:${splitSec[0]}`
        ...
        thumbIcon.addEventListener("click", () => {
          ...
          if (
            ...
            fetch(endpoint + `likes`, {
              method: "POST",
              body: JSON.stringify({ postId: arrItem._id }),
              headers: {
